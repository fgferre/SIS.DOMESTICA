...
interface LedgerRowProps {
  ...
  onEditPayments: (month: number) => void;
  onActionsMenu: (month: number, anchor: HTMLElement) => void;
...
export const LedgerRow = memo(function LedgerRow({
  ...
  const { activeYear, setTargetNet, toggleVacation, toggle13th } = usePayrollStore();
  ...
  const statusPill = (
    <button
      onClick={() => onEditPayments(entry.month)}
      className={cn(
    ...
    <div className="relative group">
      ...
      >
        ...
        <div className="col-span-3 flex items-center gap-4 md:min-w-[280px]">
          <button
            ...
            onClick={() => onToggleExpand(entry.month)}
          ...
          <div className="min-w-0">
            ...
            <div className="month-chip-row flex items-center gap-1 mt-1 flex-nowrap overflow-x-auto custom-scrollbar min-w-0 pr-2">
              ...
              {compactMode &&
                ...
                dueBadge(
                  'thirteenth',
                  isOverdue13th ? 'overdue' : is13thLatePaid ? 'late' : is13thPaid ? 'ok' : 'due',
                  thirteenthDueDate
        ...
        <div className="col-span-1 flex justify-center gap-1 text-slate-600 dark:text-slate-500">
          <button
            ...
            onClick={e => {
              e.stopPropagation();
              if (actionsMenuOpen) onCloseActionsMenu();
              else onActionsMenu(entry.month, e.currentTarget);
            }}
            className={cn(
      ...
      <AnimatedCollapse open={isExpanded} durationMs={260}>
        <div className="glass-panel border-t-0 rounded-b-lg p-6 mt-[-5px] pt-8 grid grid-cols-1 gap-6 relative z-10 bg-white/60 dark:bg-black/60 data-stream-bg border-b-2 border-b-primary/30">
          ...
          <AuditPanel entry={entry} viewMode={viewMode} dependents={dependents} />
        </div>
      </AnimatedCollapse>
    </div>
  );
