...
export function PaymentsManager({ month, isOpen, onClose }: PaymentsManagerProps) {
  ...
  const terminationDate = employee.terminationDate ? parseISODateLocal(employee.terminationDate) : undefined;
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Pagamentos Realizados - ${activeYear}/${month}`}>
      <div className="space-y-6">
        ...
        <div className="p-4 border border-white/10 rounded-md bg-white/5 space-y-3">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-1">Observação (opcional)</label>
            <input
              type="text"
              value={note}
